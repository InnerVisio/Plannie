    ...
    <div className={`min-h-screen transition-colors duration-700 ${bgClass}`}>
      <div className="max-w-6xl mx-auto space-y-6 px-4 py-8 pb-12">
        ...
        <div className="bg-white/[0.08] border border-white/20 shadow-[0_20px_50px_rgba(0,0,0,0.3)] rounded-3xl overflow-hidden">
          {/* Calendar Controls */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between px-6 py-5 border-b border-white/10 bg-white/5 gap-4">
            <h2 className="text-xl font-black text-white tracking-tight capitalize">
              {viewMode === 'calendar' ? format(currentDate, 'LLLL yyyy', { locale: cs }) : 'Všechny příspěvky'}
            </h2>
            <div className="flex items-center justify-between sm:justify-end gap-4 w-full sm:w-auto">
              ...
              <div className="flex items-center bg-white/10 rounded-xl p-1">
                <button
                  ...
                  className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-bold transition-all ${
                    viewMode === 'calendar' ? 'bg-indigo-600 text-white shadow-md' : 'text-white/60 hover:text-white hover:bg-white/5'
                  }`}
                >
              ...
              {/* Month Nav - Only in calendar mode */}
              {viewMode === 'calendar' && (
                ...
                {calendarDays.map((day, idx) => {
                  ...
                  return (
                    <div 
                      ...
                      onClick={() => {
                        if (currentUser) {
                          // Let's set time to roughly noon to avoid timezone date shifts if it defaults to midnight
                      ...
                      className={`min-h-[100px] md:min-h-[120px] p-3 transition-all duration-500 bg-white/5 hover:bg-white/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/50 group/cell border-b md:border-b-0 border-white/5 min-w-0 ${currentUser ? 'cursor-pointer hover:bg-white/20' : ''} ${
                    ...
                    >
                      <div className="flex justify-between items-start mb-3">
                        <span className={`text-xs font-black w-8 h-8 flex items-center justify-center rounded-xl transition-all duration-500 shrink-0 ${
                          isToday 
                        ...
                        <div className="flex flex-col items-end gap-1">
                          {holidayName && (
                            <span 
                              className="text-[8px] sm:text-[9px] font-bold text-rose-400/80 uppercase tracking-widest text-right line-clamp-2 max-w-[70px] sm:max-w-[100px] leading-tight group-hover/cell:text-rose-300"
                              title={holidayName}
                            >
                              {holidayName}
                            </span>
                          ...
                          {customEventsForDay.map(ev => (
                            <span 
                              key={ev.id}
                              className="text-[8px] sm:text-[9px] font-bold text-violet-400/80 uppercase tracking-widest text-right line-clamp-2 max-w-[70px] sm:max-w-[100px] leading-tight group-hover/cell:text-violet-300"
                              title={ev.name}
                            >
                              {ev.name}
                            </span>
